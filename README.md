...
It first does a SHA-256 hash of the passed key and divides the bytes of the file in blocks of 32 bytes. If the last block is smaller than 32 bytes, it adds padding until the number is reached. Then it do the XOR operations to every block with the key and puts the result to a single bytearray. Finally, it encodes the bytearray in Base64 and writes the file.
